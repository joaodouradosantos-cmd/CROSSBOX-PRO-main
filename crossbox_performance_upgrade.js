...
(function crossboxPerformanceUpgrade() {
  ...
  // Esperar que o script original termine de definir renderPerformance
  let patchAttempts = 0;
  function patchRenderPerformance() {
    if (typeof renderPerformance !== "function") {
      if (patchAttempts++ < 20) setTimeout(patchRenderPerformance, 200);
      return;
    }
    ...
    // Injectar estrutura e chamar uma primeira vez
    injectKpiCards();
    window.renderPerformance();
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", patchRenderPerformance);
  } else {
    // DOM já pronto mas scripts ainda podem estar a carregar
    setTimeout(patchRenderPerformance, 300);
  }
